pipeline {
    ...
    parameters {
        ...
        booleanParam(name: 'PACKAGE_ONLY', defaultValue: true, description: 'Only package and deploy to JFrog; skip Docker and Rundeck deployment')
        string(name: 'HARBOR_PROJECT', defaultValue: 'library', description: 'Harbor project name')
        string(name: 'IMAGE_REPOSITORY', defaultValue: 'parent', description: 'Harbor repository name without tag')
